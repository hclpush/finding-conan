...
for year, pages in pages_per_year.items():
    ...
    for i in range(2, pages + 1):
        url = f"https://www.berlin.de/polizei/polizeimeldungen/archiv/2022/?page_at_1_0={i}"
        urls.append(url)
...
for url in urls:
    html = requests.get(url).text  # replace with your url
    ...
    for police_case in urls_each_case:
        try:
            to_save = {}
            html = requests.get(police_case).text  # replace with your url
            soup = BeautifulSoup(html, "html.parser")
            ...
            date = main_content.find("p", {"class": "polizeimeldung"}).text.split(" ")[
                -1
            ]
            subtitle = (
            ...
            for paragraph in paragraphs:
                if paragraph.strong is not None:
                    ...
                    # Remove number and get description
